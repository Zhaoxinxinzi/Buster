...
# python generate_images.py -f=metrics/captions_10000.txt -t={HF_TOKEN} -e={WANDB_RUNPATH} -o={OUTPUT_FOLDER}
...
def main():
    ...
    model_path = 'path_to/model/stable-diffusion-v1-4'

    vae = AutoencoderKL.from_pretrained(model_path,subfolder="vae")

    # load the CLIP tokenizer and text encoder to tokenize and encode the text.
    tokenizer = CLIPTokenizer.from_pretrained("/home/zhaoxin/code/model/openai/clip-vit-large-patch14")
    if args.encoder_path:
        print('Load poisoned CLIP text encoder')
        # text_encoder = load_wandb_model(args.encoder_path, replace=False)
        text_encoder = CLIPTextModel.from_pretrained(args.encoder_path)

    else:
        print('Load clean CLIP text encoder')
        text_encoder = CLIPTextModel.from_pretrained(
            "/home/zhaoxin/code/model/openai/clip-vit-large-patch14")
    ...
    for step in tqdm(range(max_iterations)):
        ...
        latents = torch.randn(
            (len(batch), unet.config.in_channels, args.height // 8, args.width // 8),
            generator=generator,
        )
